# treatPDF_sizing実行用シェルスクリプト
python treatPDF_sizing.py ../models/obj/bishop.obj << EOF
2.1
126 113 143
y
./../photo/Design/ビショップ-黄色.png
0.00
2.765
1.95
n
n
n
0.1
bishop_21_purple
EOF

python treatPDF_sizing.py ../models/obj/king.obj << EOF
2.1
126 113 143
y
./../photo/Design/キング-黄色.png
0.00
2.765
1.95
n
n
n
0.1
king_21_purple
EOF

python treatPDF_sizing.py ../models/obj/knight.obj << EOF
2.1
126 113 143
y
./../photo/Design/ナイト-黄色.png
0.00
2.765
1.95
n
n
n
0.1
knight_21_purple
EOF

python treatPDF_sizing.py ../models/obj/pawn.obj << EOF
2.1
126 113 143
y
./../photo/Design/ポーン-黄色.png
0.00
2.765
1.95
n
n
n
0.1
pawn_21_purple
EOF

python treatPDF_sizing.py ../models/obj/qween_front.obj << EOF
2.1
126 113 143
y
./../photo/Design/クイーン-黄色.png
0.00
2.765
1.95
n
n
n
0.1
qween_f_21_purple
EOF

python treatPDF_sizing.py ../models/obj/qween_back.obj << EOF
2.1
126 113 143
n
n
n
n
qween_b_21_purple
EOF

python treatPDF_sizing.py ../models/obj/rook.obj << EOF
2.1
126 113 143
y
./../photo/Design/ルーク-黄色.png
0.00
2.765
1.95
n
n
n
0.1
rook_21_purple
EOF

python treatPDF_sizing.py ../models/obj/king_tall.obj << EOF
2.1
126 113 143
y
./../photo/Design/キング-黄色.png
0.00
2.765
1.95
n
n
n
0.1
king_tall_21_purple
EOF

python treatPDF_sizing.py ../models/obj/qween_front_tall.obj << EOF
2.1
126 113 143
y
./../photo/Design/クイーン-黄色.png
0.00
2.765
1.95
n
n
n
0.1
qween_f_tall_21_purple
EOF

python treatPDF_sizing.py ../models/obj/qween_back_tall.obj << EOF
2.1
126 113 143
n
n
n
n
qween_b_tall_21_purple
EOF